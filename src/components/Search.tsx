...
export default function Search() {
  ...
  const {removeFromAllUsers ,filterAllUser ,filteredUsers , addToAllUsers} = useAllUsers();
  const {addToSelected , selectedUsers , removeFromSelected}=useSelectedUsers()
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement> | React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (onBackSpace && (e as React.KeyboardEvent<HTMLInputElement>).code === "Backspace") {
    ...
    if (input === "" && (e as React.KeyboardEvent<HTMLInputElement>).code === "Backspace") {
  ...
  return (
    <div className="relative">
      <input value={query} placeholder="Add new User" onKeyUp={handleSearch} onChange={handleSearch} className={`${onBackSpace && "font-extrabold border-red-300  text-blue-500 bg-black"} p-2`} onFocus={()=>setShowList(true)} />
      {onBackSpace && <span onClick={()=>{setQuery("");setOnBackspace(false)}} className="absolute cursor-pointer right-[-20] bottom-7 bg-slate-300 rounded-full"><IoMdClose size={15}/></span>}
      ...
      <span className=' text-end flex justify-end items-center w-[20px] text-xl hover:cursor-pointer rounded-full' onClick={()=>setShowList(false)}><IoMdClose/></span>
       {  filteredUsers.length===0  ? <p className="w-auto">Nothing to select</p>:
        filteredUsers.map((user) => (
          <div key={user.userName} onClick={()=>handleAdd(user)} className="p-2 flex flex-col sm:flex-row justify-between items-center gap-4  hover:cursor-pointer hover:bg-slate-100 rounded-md">
